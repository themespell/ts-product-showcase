...
function initializeAllWidgets() {
    const showcaseElements = document.querySelectorAll('.ts-product-showcase');
...
function Frontend({ id }) {
    ...
    const devMode = Boolean(tsproduct_settings.devmode);
        ...
        if (id) {
            fetchData(`tsproduct/product_showcase/fetch/single`, (response) => {
                if (response && response.success) {
                    setTeamMembers(response.data.meta_data.products);
