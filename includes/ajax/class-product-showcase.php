...
class ProductShowcase {
	...
	public static function init() {
		$self = new self();
		// Team Showcase Ajax
		add_action( 'wp_ajax_tsproduct/product_showcase/fetch', array( $self, 'get_showcase' ) );

		add_action( 'wp_ajax_tsproduct/product_showcase/fetch/single', array( $self, 'get_showcase_by_id' ) );
		add_action( 'wp_ajax_nopriv_tsproduct/product_showcase/fetch/single', array( $self, 'get_showcase_by_id' ) );
		
		add_action( 'wp_ajax_tsproduct/product_showcase/create', array( $self, 'create_showcase' ) );
		add_action( 'wp_ajax_tsproduct/product_showcase/update', array( $self, 'update_showcase' ) );
		add_action( 'wp_ajax_tsproduct/product_showcase/update/settings', array( $self, 'update_showcase_settings' ) );
		add_action( 'wp_ajax_tsproduct/product_showcase/duplicate', array( $self, 'duplicate_showcase' ) );
		add_action( 'wp_ajax_tsproduct/product_showcase/delete', array( $self, 'delete_showcase' ) );
	}
	...
	public function get_showcase() {
		...
		while ( $query->have_posts() ) {
			...
			$showcases[] = array(
				'post_id'   => get_the_ID(),
				'title'     => get_the_title(),
				'shortcode' => '[ts_product_showcase id="' . get_the_ID() . '"]',
				'snippet'  => '<?php echo do_shortcode(\'[ts_product_showcase id="' . get_the_ID() . '"]\'); ?>',
			);
		}
